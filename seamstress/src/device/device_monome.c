...
int dev_monome_init(void *self) {
  ...
  if(md->rows == 0 && md->cols == 0) {
    fprintf(stderr, "monome device reports zero rows/cols; assuming arc\n");
...
static inline uint8_t dev_monome_quad_index(uint8_t x, uint8_t y) {
  return ((y > 7) << 1) | (x > 7);
}
...
void dev_monome_grid_set_led(struct dev_monome *md, uint8_t x, uint8_t y, uint8_t val) {
  uint8_t q = dev_monome_quad_index(x, y);
...
void dev_monome_intensity(struct dev_monome *md, uint8_t intensity) {
  if (intensity > 15) {
    intensity = 15;
  }
  monome_led_intensity(md->m, intensity);
}
...
static inline void arc_key_event(const monome_event_t *e, void *p, int state) {
  struct dev_monome *md = (struct dev_monome *)p;
...
void dev_monome_handle_encoder_delta(const monome_event_t *e, void *p) {
  struct dev_monome *md = (struct dev_monome *)p;
  union event_data *ev = event_data_new(EVENT_ARC_ENCODER);
